...
const Card = ({event,hasOrderLink,hidePrice}: CardPropsType) => {
  ...
  return (
    <div className='group relative flex min-h-[380px] w-full max-w-[400px] flex-col overflow-hidden rounded-xl bg-white shadow-md transition-all hover:shadow-lg md:min-h-[480px]'>
        ...
        {/* ------Below section will be diplayed if it is an event creator----- */}
        {isEventCreator && !hidePrice && (
          <div className='absolute right-2 top-2 flex flex-col gap-4 rounded-xl bg-transparent hover:bg-white p-2 shadow-sm transition-all'>
        ...
        {/*--------END---------*/}
        <Link href={`/events/${event._id}`}  className='flex min-h-[200px] flex-col gap-3 p-5 md:gap-4'>
